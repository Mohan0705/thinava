    ...
    >
      <div className="space-y-6">
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          <Card><CardContent className="p-5"><p className="text-sm text-slate-500">Active</p><p className="mt-2 text-3xl font-bold">{ordersQuery.data?.summary.active ?? 0}</p></CardContent></Card>
          <Card><CardContent className="p-5"><p className="text-sm text-slate-500">Delayed</p><p className="mt-2 text-3xl font-bold text-amber-600">{ordersQuery.data?.summary.delayed ?? 0}</p></CardContent></Card>
          <Card><CardContent className="p-5"><p className="text-sm text-slate-500">Cancelled</p><p className="mt-2 text-3xl font-bold text-rose-600">{ordersQuery.data?.summary.cancelled ?? 0}</p></CardContent></Card>
          <Card><CardContent className="p-5"><p className="text-sm text-slate-500">COD Orders</p><p className="mt-2 text-3xl font-bold">{ordersQuery.data?.summary.cod ?? 0}</p></CardContent></Card>
        </div>
        ...
        <Card className="border border-white/70 bg-white/90">
          ...
          <CardContent className="grid gap-3 md:grid-cols-2 xl:grid-cols-5">
            <select className="h-12 rounded-xl border-2 border-gray-200 bg-white px-4" value={filters.status} onChange={(event) => setFilters((current) => ({ ...current, status: event.target.value }))}>
              <option value="">All statuses</option>
              {ordersQuery.data?.filters.statuses.map((status) => <option key={status} value={status}>{status.replace(/_/g, ' ')}</option>)}
            </select>
            <select className="h-12 rounded-xl border-2 border-gray-200 bg-white px-4" value={filters.restaurant} onChange={(event) => setFilters((current) => ({ ...current, restaurant: event.target.value }))}>
            ...
            <select className="h-12 rounded-xl border-2 border-gray-200 bg-white px-4" value={filters.rider} onChange={(event) => setFilters((current) => ({ ...current, rider: event.target.value }))}>
              <option value="">All riders</option>
              {ordersQuery.data?.filters.riders.map((rider) => <option key={rider.id} value={rider.id}>{rider.name}</option>)}
            </select>
            <select className="h-12 rounded-xl border-2 border-gray-200 bg-white px-4" value={filters.area} onChange={(event) => setFilters((current) => ({ ...current, area: event.target.value }))}>
              <option value="">All areas</option>
              {ordersQuery.data?.filters.areas.map((area) => <option key={area} value={area}>{area}</option>)}
            </select>
            <select className="h-12 rounded-xl border-2 border-gray-200 bg-white px-4" value={filters.payment_method} onChange={(event) => setFilters((current) => ({ ...current, payment_method: event.target.value }))}>
        ...
        <div className="space-y-4">
          {ordersQuery.data?.orders.map((order) => {
            ...
            return (
            <Card key={order.id} className={`border ${isTerminal ? 'border-slate-200 opacity-70' : 'border-white/70'} bg-white/90`}>
              <CardContent className="p-5">
                <div className="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
                  ...
                  <div className="flex flex-wrap gap-2 xl:max-w-[340px] xl:justify-end">
                    ...
                    <a href={`tel:${order.customer.phone}`}><Button variant="secondary">Call Customer</Button></a>
                    {order.rider?.phone ? <a href={`tel:${order.rider.phone}`}><Button variant="secondary">Call Rider</Button></a> : null}
                  </div>
                </div>
              </CardContent>
            </Card>
