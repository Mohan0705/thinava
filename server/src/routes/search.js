...
const { applyAvailabilityToRestaurants } = require('../utils/restaurantAvailability')
...
router.get('/by-category/:category', asyncHandler(async (req, res) => {
  ...
  try {
    ...
    const query = `
      ...
      AND UPPER(COALESCE(r.status, 'OPEN')) NOT IN ('REJECTED', 'PENDING', 'PENDING_APPROVAL')
      ORDER BY r.name ASC
      LIMIT 100
    `

    const result = await pool.query(query, [normalizedCategory])
    const restaurants = applyAvailabilityToRestaurants(
      (result.rows || []).filter(r => r && typeof r === 'object')
    )
...
router.get('/', asyncHandler(async (req, res) => {
  ...
  try {
    ...
    const restaurants = applyAvailabilityToRestaurants(
      (restaurantsResult.rows || []).filter(r => r && typeof r === 'object')
    )
    ...
    if (menuItemRestaurantIds.length > 0) {
      ...
      restaurantsFromMenuItems = applyAvailabilityToRestaurants(
        (menuRestResult.rows || []).filter(r => r && typeof r === 'object')
      )
    ...
    const combinedRestaurants = applyAvailabilityToRestaurants(Array.from(restaurantMap.values()))
